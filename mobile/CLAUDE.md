...
- **`src/api/client.ts`** — fetch wrapper for the backend. `sendChat(message, lang)` POSTs to `/chat` and returns a `ChatResponse` with `status: "answered" | "unknown"` plus `citations`. `checkHealth()` polls `/health` and drives the offline banner on the home screen (checked on screen change and on app-foreground via `AppState`). `fetchMaxQueryLength()` reads `/health`'s `max_query_length` once on mount to sync the input's max length with the backend's `CHATBOT_MAX_QUERY_LENGTH`, falling back to a local default if the fetch fails.
...
Citations are only rendered when `status === "answered"` and `citations.length > 0`; `status === "unknown"` suppresses citation links even if present.
