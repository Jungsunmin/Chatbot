...
def chat(req: ChatRequest):
    ...
    retrieval = retriever.search_with_band(query, intent=intent)

    if retrieval.band in ("none", "low") or not retrieval.docs:
        return _unknown_response(req.lang)

    if retrieval.band == "high":
        answer, used_docs, model_used = _build_answer(retrieval.docs, req.lang, query)
        if answer == unknown_message(req.lang):
            return _unknown_response(req.lang)
        return ChatResponse(
            status="answered",
            answer=answer,
    ...
    pending_id = create_pending(query, req.lang, retrieval.docs)
    preview = compose_confirm_preview(retrieval.docs, req.lang)
    prompt = confirm_prompt_text(req.lang)
    return ChatResponse(
